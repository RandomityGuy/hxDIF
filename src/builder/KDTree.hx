...
@:expose("builder.KdTree")
class KdTree<T> {

    private var points : Array<KdPoint<T>>;
    private var distanceFunction : Point3F -> Point3F -> Float;
    ...
    private function buildTree(points : Array<KdPoint<T>>, depth : Int, parent : KdNode<T>) : KdNode<T> {
        ...
        points.sort(function(a : KdPoint<T>, b : KdPoint<T>) {
            var diff = a.point.get(dim) - b.point.get(dim);
            if (diff == 0.0){
                return 0;
            } else if (diff > 0) {
                return 1;
    ...
    public function nearest(point : Point3F, maxNodes : Int, maxDistance : Float ) : Array<Pair<KdPoint<T>, Float>> {
        ...
        function nearestSearch(node : KdNode<T>) {
            ...
            for (i in 0...dim){
                if (i == node.dimension) {
                    linearPoint[i] = point.get(dim);
                } else {
                    linearPoint[i] = node.kdPoint.point.get(i);
                }
            }

            linearDistance = new Point3F(linearPoint[0],linearPoint[1],linearPoint[2]).sub(node.kdPoint.point).length();
            ...
            if (node.right == null) {
                ...
                if (point.get(dimension) < node.kdPoint.point.get(dimension)) {
                    bestChild = node.left;
                } else {
                    bestChild = node.right;
                }
        ...
        for (i in 0...maxNodes){
            if (bestNodes.content[i].item0 != null) {
                bestNodes.content[i].item0.used = true;
